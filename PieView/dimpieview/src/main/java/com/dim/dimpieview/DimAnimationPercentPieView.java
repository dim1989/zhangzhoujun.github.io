...
public class DimAnimationPercentPieView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (sum <= 0) {
            if (mListener != null) {
                mListener.onDrawPiePercent(sum, true);
            }
            return;
        }
        if (positionList != null && positionList.size() > 0) {
            ...
            for (int i = 0; i < positionList.size(); i++) {
                ...
                if (currentAngle >= arcInfo.getStartAngle()
                    && currentAngle <= arcInfo.getStartAngle() + arcInfo.getAngle()) {
            ...
            if (currentAngle <= getTotalNeedDrawAngle()) {
                if (mListener != null) {
                    mListener.onDrawPiePercent(currentAngle * 100 / 360, false);
                }
                invalidate();
            } else {//绘制扇形完毕，绘制数据
                if (mListener != null) {
     ...
     */
    public void setData(long[] numbers, String[] names, OnDImAnimalPercentPieViewListener listener) {
     ...
     * @param listener 绘制进度的回掉
     */
    public void setData(long[] numbers, String[] names, int[] colors, OnDImAnimalPercentPieViewListener listener) {
        if (numbers == null || numbers.length == 0
